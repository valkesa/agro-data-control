...
Future<void> main(List<String> args) async {
  ...
  if (timeoutMs <= 0) {
    stderr.writeln('Error: --timeout-ms debe ser positivo.');
    exitCode = 64;
    return;
  }

  final _SimpleModbusWriter client = _SimpleModbusWriter(
    host: host,
    port: port,
    unitId: unitId,
  ...
  try {
    stdout.writeln('writing coil $address = $value');
    ...
    stderr.writeln(
      'Error de conexion al PLC ($host:$port): timeout al conectar o escribir.',
    );
    exitCode = 1;
  } on _SimpleModbusException catch (error) {
    stderr.writeln('Error Modbus escribiendo coil: ${error.message}');
    ...
    await client.close(reason: 'logo_write_coil_done');
  }
}
...
class _SimpleModbusWriter {
  ...
  Future<void> close({String reason = 'client_close'}) async {
    ...
    if (reason.isNotEmpty) {
      // no-op, kept only to make close call sites explicit
    }
  }
  ...
  Future<void> writeSingleCoil({
    ...
    if (response.length < 5) {
      throw _SimpleModbusException('Respuesta demasiado corta');
    }
    if (response[0] != 0x05) {
      throw _SimpleModbusException(
        'Function code inesperado en respuesta: ${response[0]}',
      );
    }
